...
resource "equinix_network_device" "this" {
  ...
  vendor_configuration = {
    activationKey  = var.activation_key
    controllerFqdn = var.controller_fqdn
    rootPassword   = var.root_password != "" ? var.root_password : null
  }
  dynamic "secondary_device" {
    ...
    content {
      ...
      vendor_configuration = {
        activationKey  = var.secondary.activation_key
        controllerFqdn = var.secondary.controller_fqdn
        rootPassword   = try(var.secondary.root_password, null)
      }
    }
  }
}
